...
| **`orchestration/ORCHESTRATION.md`** | 编排手册：工具映射、派活 prompt 模板、里程碑派活清单、**§5 硬规则 1-23** |
...
orchestration/        ORCHESTRATION.md(编排手册·硬规则1-23) · kesheng_workflow.js+.meta.json(全自动 fan-out)
protocols/            meeting.md(会议协作协议) · quality-gate.md(三态门控+检察官五层)
playbooks/ (7)        production-workflow(端到端) · asset-library-flow(素材库→参考图) ·
                      ...
                      consistency(跨镜一致性)
templates/ (20)       brief · proposal · storyboard · decision-log · breakdown · handoff ·
                      prompt-sheet(格式规范) · prompt-sop(写prompt十步) · product/video-prompt-formula ·
...
→ KSP-04.5 九宫格风格预览 → 风格基线固化       ▲拍板#15 风格预览
      ★ 顺序铁则：口播稿 → 九宫格 → 才做分镜
→ KSP-06 执行(分镜‖声音 → 摄影 → prompt分段‖剪辑 → 美术VI复核 → 科学复核)
      → check_all.py(8项) → 红队前置闸 → 出口闸L4 → 检察官五层 → 交付(两档) → L5 签收
